...
def plot_results(contended_results: list,
    ...
    # ===============================================================
    # MODULE 5 - SEPARATE FIGURES (CLEAN)
    # ===============================================================

    # --- Metrics ---
    base_time = k_times[0]

    speedup_c = [base_time / t for t in c_times]
    speedup_k = [base_time / t for t in k_times]

    eff_c = [s / n for s, n in zip(speedup_c, n_vals)]
    eff_k = [s / n for s, n in zip(speedup_k, n_vals)]

    c_util = [r["bus_util"] * 100 for r in contended_results]
    k_util = [r["bus_util"] * 100 for r in controlled_results]

    p = 0.1
    amdahl = [1 / ((1 - p) + (p / n)) for n in n_vals]
    
    c_mem = [r["memory"] for r in contended_results]
    k_mem = [r["memory"] for r in controlled_results]
    
    print("\n" + "=" * 80)
    print(" MODULE 5 - PERFORMANCE METRICS SUMMARY")
    print("=" * 80)

    print(f"{'Processors':<12}{'Speedup(C)':<15}{'Speedup(K)':<15}"
          f"{'Efficiency(C)':<15}{'Efficiency(K)':<15}")
    ...
    for n, a in zip(n_vals, amdahl):
        print(f"Processors {n:<2} → Theoretical Speedup: {a:.4f}")

    print("\nCPU UTILISATION (%)")
    print("-" * 40)

    print(f"{'Processors':<12}{'Contended':<15}{'Controlled':<15}")

    for i, n in enumerate(n_vals):
        print(f"{n:<12}{c_util[i]:<15.2f}{k_util[i]:<15.2f}")
    ...
    for i, n in enumerate(n_vals):
        print(f"{n:<12}{c_mem[i]:<15.2f}{k_mem[i]:<15.2f}")

    print("=" * 80)

    # ---------------- SPEEDUP ----------------
    plt.figure(figsize=(6, 4))
    plt.plot(n_vals, speedup_c, "o-", label="Contended")
    plt.plot(n_vals, speedup_k, "s-", label="Controlled")
    plt.plot(n_vals, amdahl, "--", label="Amdahl (p=0.1)")
    plt.title("Speedup vs Processors")
    plt.xlabel("Processors")
    plt.ylabel("Speedup")
    plt.legend()
    plt.grid(True)
    plt.savefig("module5_speedup.png", dpi=150, bbox_inches="tight")


    # ---------------- EFFICIENCY ----------------
    plt.figure(figsize=(6, 4))
    plt.plot(n_vals, eff_c, "o-", label="Contended")
    plt.plot(n_vals, eff_k, "s-", label="Controlled")
    plt.title("Efficiency vs Processors")
    plt.xlabel("Processors")
    plt.ylabel("Efficiency")
    plt.legend()
    plt.grid(True)
    plt.savefig("module5_efficiency.png", dpi=150, bbox_inches="tight")


    # ---------------- CPU UTILISATION ----------------
    plt.figure(figsize=(6, 4))
    plt.plot(n_vals, c_util, "o-", label="Contended")
    plt.plot(n_vals, k_util, "s-", label="Controlled")
    plt.title("CPU Utilisation vs Processors")
    plt.xlabel("Processors")
    plt.ylabel("Utilisation (%)")
    plt.legend()
    plt.grid(True)
    plt.savefig("module5_cpu.png", dpi=150, bbox_inches="tight")


    # ---------------- EXECUTION TIME ----------------
    # --- Sequential baseline (N=1) ---
    seq_time = k_times[0]  # controlled N=1
    seq_line = [seq_time] * len(n_vals)

    # --- Plot ---
    plt.figure(figsize=(6, 4))

    # Parallel curves
    plt.plot(n_vals, c_times, "o-", label="Parallel (Contended)")
    plt.plot(n_vals, k_times, "s-", label="Parallel (Controlled)")

    # Sequential reference line
    plt.plot(n_vals, seq_line, "--", label="Sequential (1 Processor)")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("module5_execution_time.png", dpi=150, bbox_inches="tight")
    
    
    # ---------------- Memory Usage ----------------
    plt.figure(figsize=(6, 4))
    plt.plot(n_vals, c_mem, "o-", label="Contended")
    plt.plot(n_vals, k_mem, "s-", label="Controlled")
    plt.title("Memory Usage vs Processors")
    plt.xlabel("Processors")
    plt.ylabel("Memory (MB)")
    plt.legend()
    plt.grid(True)
    plt.savefig("module5_memory.png", dpi=150)

    print("[Module 5] All graphs saved separately")
